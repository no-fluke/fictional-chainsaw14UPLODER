...
async def _add_user(client, message):
    await add_user_cmd(client, message)

@bot.on_message(filters.command("remove") & filters.private)
async def _remove_user(client, message):
    await remove_user_cmd(client, message)
...
async def add_auth_user(client: Client, message: Message):
    ...
    try:
        ...
        success, expiry = db.add_user(new_user_id, name, 30, bot_info.username)
        if success:
            await message.reply_text(
                f"**✅ User `{new_user_id}` added for 30 days.**\n"
                f"Use `/add {new_user_id} <days>` to set a custom duration."
            )
        else:
        ...
        for i in range(arg-1, len(links)):
            ...
            if "jw-prod" in url:
               ...
               cmd = f'yt-dlp --add-header "referer:https://web.classplusapp.com/" --add-header "x-cdn-tag:empty" -f "{ytf}" "{url}" -o "{name}.mp4"'
                ...
                cmd = f'yt-dlp --cookies youtube_cookies.txt --extractor-args "youtube:player_client=tv_embedded,ios" -f "{ytf}" "{url}" -o "{name}.mp4"'
            else:
